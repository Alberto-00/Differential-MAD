...
def mergin_models():
    ...
    # Dividiamo il dataset in set di training e test
    X_train, X_test, y_train, y_test = train_test_split(iris.data, iris.target, test_size=0.2, random_state=42)

    # Definiamo tre modelli di classificazione diversi
    model1 = DecisionTreeClassifier(random_state=42)
    model2 = KNeighborsClassifier(n_neighbors=5)
    model3 = LogisticRegression(random_state=42)

    # Creiamo un ensemble di modelli
    ensemble = VotingClassifier(estimators=[('dt', model1), ('knn', model2), ('lr', model3)], voting='hard')

    # Addestriamo l'ensemble sui dati di training
    ensemble.fit(X_train, y_train)

    # Effettuiamo la previsione sull'insieme di test
    y_pred = ensemble.predict(X_test)

    # Valutiamo le performance dell'ensemble
